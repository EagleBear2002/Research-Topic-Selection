...
<!-- 研究范式不仅影响兴趣，还决定了成果的可信度和意义感。工程驱动领域复现难度大，理论领域则强调严密性。大家要理性看待不同方向的优劣。 -->

<!-- _class: trans -->

### 研究范式与研究成果的可信度高度相关

<!-- 工程驱动的 AI 领域存在复现危机和数据泄露等问题。我们要重视科研诚信，推动开放科学和可复现性。 -->
...
<!-- 这项研究系统性复现了 30 篇高引用 AI 论文，发现只有一半能被部分或全部复现。开放数据和代码是提升复现率的关键。 -->
...
<!-- 数据泄露是机器学习领域的严重问题，影响了大量论文的结论。修复泄漏后，复杂模型的优势大大减弱。我们要警惕数据处理中的隐性风险。 -->
...
<!-- GPTZero 检测发现顶会论文中存在大量幻觉引用，包括虚假文献和错误信息。这提醒我们在引用和写作时要格外严谨。 -->
...
<!-- 不同研究范式下，成果产出的难度和数量差异很大。AI 领域论文数量多但质量参差，理论领域则重在突破。大家要根据自身目标合理定位。 -->
